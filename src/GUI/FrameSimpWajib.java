...
public class FrameSimpWajib extends javax.swing.JInternalFrame {    
    Connection conn ;
    Statement stat;
    ResultSet rs;
    String[][]dataSimpananWajib;
    String formatTanggal = "yyyy-MM-dd";
    SimpleDateFormat sdf = new SimpleDateFormat(formatTanggal);     
       
    public FrameSimpWajib() {
        initComponents();
    ...
    private void initComponents() {
        ...
        tabelSimpanan.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID Simpanan Wajib", "ID Anggota", "Nama", "Jumlah Simpanan Wajib", "Tanggal Bayar", "Tanggal Aktif"
            }
        ) {
    ...
    private void urutan() {
      String sql = "select right(id_simp_wajib,2) as a from simpanan_wajib ORDER BY a ASC";
        try{
            stat = conn.prepareStatement(sql);
            rs = stat.executeQuery(sql);
            if (rs.first()==false){
               txtIdSimpanan.setText("SW01");
            }else{
                rs.last();
    ...
    private ResultSet dataSimpananWajib(){
        try{
            String SQL = "SELECT * from simpanan_wajib order by id_simp_wajib asc ";
    ...
    private DefaultTableModel tabelModel(){
           int rows =0;
           String [] namaKolom={"ID","ID Anggota","Nama","Jumlah SW","Tanggal Bayar","Tanggal Aktif"};
           int baris = 0;
           try{
               while(rs.next()){
                   baris = rs.getRow();
               ...
               rs.beforeFirst();
               dataSimpananWajib = new String[baris][namaKolom.length];
               for(int i = 0; i < baris; i++){
                   rs.next();
                    for(int j=0; j < namaKolom.length;j++){
                        dataSimpananWajib[i][j] =rs.getString(j+1);
                    }
               }
           }catch (Exception ex) {}
           return new DefaultTableModel(dataSimpananWajib,namaKolom);
       }
    
    public void tabelDataSimpananWajib(){
        try{
            rs = dataSimpananWajib();
    ...
    */
    ...
    private void simpanData() {       
       String idSimWajib = txtIdSimpanan.getText();
       ...
       String tglBerlaku  = String.valueOf(sdf.format(tanggalBerlaku.getDate()));     
        ...
        if(idSimWajib.equals("") || nama.equals("") || idAnggota.equals("") || jumlahSp.equals("") || tglBayar.isEmpty() || tglBerlaku.isEmpty() )
        {
        ...
        {
            try{
                String SQL = " INSERT INTO simpanan_wajib VALUES"
                        +"('" + idSimWajib + "','" 
                        + idAnggota + "','"
                        + nama + "','"
                        + jumlahSp + "','"
    ...
    private void ubahData() {
        ...
        try{
                String SQL = "update simpanan_wajib set id_anggota=?, nama_anggota=?, Jumlah_sw=?, createDate=?,forDate=?"                       
                        + "WHERE id_simp_wajib ='" + id + "'";
    ...
    private void hapusdata() {
        String id = txtIdSimpanan.getText();
        try{
            String SQL = "DELETE FROM simpanan_wajib WHERE id_simp_wajib ='" + id + "'";
            stat=conn.createStatement();
            int data = stat.executeUpdate(SQL);
            if(data != 0 )
            {
